...
class SupplierUpdateButton extends StatelessWidget {
  const SupplierUpdateButton({required this.supplier, super.key});
  ...
  Widget build(BuildContext context) {
    return IconButtonSmall(
      permission: Permission.supplierEdit,
      action: DataAction.edit,
      onPressed: () async {
        final queryBloc = context.read<SupplierQueryBloc>();
        final success = await Navigator.push(
          context,
          SupplierCreatePage.route(supplier: supplier),
        );
        if (success ?? false) {
