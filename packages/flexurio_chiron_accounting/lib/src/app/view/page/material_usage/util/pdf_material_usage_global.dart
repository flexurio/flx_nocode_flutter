...
Future<Page> pdfMaterialUsageGlobal({
  required List<MaterialUsageGlobal> data,
}) async {
  ...
  return MultiPage(
    ...
    build: (Context context) {
      return [
        simpleTablePdf<MaterialUsageGlobal>(
          data: data,
          columns: [
            PColumn(
              title: 'MI.No',
              contentBuilder: (data, index) {
                return data.materialIssueId;
              },
            ),
            PColumn(
              title: 'date'.tr(),
              contentBuilder: (data, index) {
                return DateFormat('dd/MM/yyyy').format(data.materialIssueDate);
              },
            ),
            PColumn(
