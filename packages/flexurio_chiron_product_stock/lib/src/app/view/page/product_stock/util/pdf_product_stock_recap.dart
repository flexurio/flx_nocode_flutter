import 'package:appointment/src/app/resource/user_repository.dart';
import 'package:easy_localization/easy_localization.dart';
import 'package:flexurio_chiron_accounting/src/app/model/product_stock_recap.dart';
import 'package:flexurio_erp_core/flexurio_erp_core.dart';
import 'package:pdf/pdf.dart';
import 'package:pdf/widgets.dart';
import 'package:printing/printing.dart';

const paddingRow = EdgeInsets.symmetric(vertical: 6, horizontal: 6);
final format = NumberFormat('#,##0.00', 'en_US');

Future<Page> pdfRecapProductStock({
  required List<ProductStockRecap> data,
}) async {
  final companyLogo = await getCompanyLogoPdf();
  final font = await PdfGoogleFonts.nunitoExtraLight();
  final printedBy = UserRepositoryApp.instance.userApp?.name ?? '';

  return MultiPage(
    theme: ThemeData(defaultTextStyle: TextStyle(font: font)),
    pageFormat: PdfPageFormat.a4,
    header: (context) => headerPdf(
      companyLogo: companyLogo.$1,
      title: 'SISA BARANG (FINISH GOODS)',
      companyLogoNamed: companyLogo.$2,
    ),
    footer: (context) => footerPdf(context: context, printedBy: printedBy),
    build: (Context context) {
      return [
        simpleTablePdf<ProductStockRecap>(
          data: data,
          columns: [
            PColumn(
              title: '${'produk'.tr()} / ${'transaction_no'.tr()}',
              contentBuilder: (data, index) {
                return '${data.product.name} ${data.productReceivedId}';
              },
            ),
            PColumn(
              title: 'expired_date'.tr(),
              contentBuilder: (data, index) {
                return DateFormat('dd/MM/yyyy').format(data.expiredDate);
              },
            ),
            PColumn(
              title: 'release_date'.tr(),
              contentBuilder: (data, index) {
                return DateFormat('dd/MM/yyyy').format(data.releaseAt);
              },
            ),
            PColumn(
              title: 'batch_no'.tr(),
              contentBuilder: (data, index) {
                return data.batchNoId;
              },
            ),
            PColumn(
              title: 'unit'.tr(),
              contentBuilder: (data, index) {
                return data.unit;
              },
            ),
            PColumn(
              title: 'remainder'.tr(),
              contentBuilder: (data, index) {
                return data.endingBalanceMarketing.format();
              },
            ),
          ],
        ),
      ];
    },
  );
}
