...
import 'package:flexurio_chiron_accounting/src/app/view/page/debt_mutation_report/util/pdf_debt_mutation_report_invoice_date.dart';
...
class DebtMutationReportInvoiceDateExportPdfButton extends StatefulWidget {
  const DebtMutationReportInvoiceDateExportPdfButton({super.key});
  ...
  static Widget prepare() {
    return MultiBlocProvider(
      ...
      child: const DebtMutationReportInvoiceDateExportPdfButton(),
    );
  }

  @override
  State<DebtMutationReportInvoiceDateExportPdfButton> createState() =>
      _DebtMutationReportInvoiceDateExportPdfButtonState();
}

class _DebtMutationReportInvoiceDateExportPdfButtonState
    extends State<DebtMutationReportInvoiceDateExportPdfButton> {
  ...
  Widget build(BuildContext context) {
    ...
    return LightButtonSmall(
      ...
      title: 'Invoice Date',
      permission: PermissionAccounting.debtMutationReportInvoiceDateExportPdf,
      onPressed: () async {
        await _showFilter(
          context,
          queryBloc,
                ...
                state.maybeWhen(
                  ...
                  loaded: (debtBalanceDetailInvoiceDate) async {
                    final pdf = pw.Document()
                      ..addPage(
                        await pdfDebtMutationReportInvoiceDate(
                          data: debtBalanceDetailInvoiceDate.data,
                        ),
                      );
                    await Printing.sharePdf(
                      bytes: await pdf.save(),
                      filename: 'Debt_Mutation_Invoice_Date.pdf',
                    );
                  },
                );
              },
