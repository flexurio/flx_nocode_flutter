...
void main() {
  ...
  group('NoCode.navigatePushMenu Initial Value Test', () {
    ...
    final mockEntityJson = {
            ...
            "actions": [
              {
                ...
                "http": {
                  "method": "POST",
                  "url":
                      "/employees/create?form_name={{ form.name }}&parent_name={{ parent.name }}",
                  "body": {
                    "nik": "{{ form.nik }}",
                    "name": "{{ form.name }}",
      ...
      "layout_table": {"nik": 1, "name": 2},
      "layout_list_tile": {"title": "{{ nik }}", "subtitle": "{{ name }}"}
    };
    ...
    setUpAll(() async {
      ...
      binaryMessenger.setMockMessageHandler('flutter/assets', (message) async {
        ...
        if (key.contains('asset/translation/en.json') ||
            key.contains('asset/translation/id.json')) {
      ...
      when(() => mockRepo.modify(
        ...
        return Response<Map<String, dynamic>>(
          ...
          data: (mockData as Map<String, dynamic>?) ?? {},
          statusCode: 200,
        );
      });
    });
    ...
    testWidgets('should show initial value from parentData in TextField',
        (tester) async {
      ...
      if (find
          .textContaining(RegExp('Add Employee', caseSensitive: false))
          .evaluate()
          .isEmpty) {
      ...
      final addButton =
          find.textContaining(RegExp('Add Employee', caseSensitive: false));
      expect(addButton, findsOneWidget);
      await tester.tap(addButton);
      await tester.pumpAndSettle();

      // 4. Verify CreatePage is shown
      expect(find.textContaining(RegExp('Create', caseSensitive: false)),
          findsAtLeast(1));
      expect(find.textContaining(RegExp('Employee', caseSensitive: false)),
          findsAtLeast(1));

      // 5. Verify TextFields have initial values from parentData
      final textFieldNik = find.descendant(
        of: find.ancestor(
            of: find.text('NIK'), matching: find.byType(core.FTextFormField)),
        matching: find.byType(TextField),
      );
      final textFieldName = find.descendant(
        of: find.ancestor(
            of: find.text('Name'), matching: find.byType(core.FTextFormField)),
        matching: find.byType(TextField),
      );
      ...
      final nameController =
          (tester.widget<TextField>(textFieldName)).controller;
      ...
      final submitButton =
          find.textContaining(RegExp('Submit', caseSensitive: false));
