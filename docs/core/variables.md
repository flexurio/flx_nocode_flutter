...
| `parent` | The immediate parent record when rendering nested child forms/sub-details (automatically populated from `parentData.last`). Allows accessing parent fields like `{{ parent.chart_of_account_id }}` in `visibilityCondition`, `initialValue`, `httpData.url`, and table action rules (`rule`). |
