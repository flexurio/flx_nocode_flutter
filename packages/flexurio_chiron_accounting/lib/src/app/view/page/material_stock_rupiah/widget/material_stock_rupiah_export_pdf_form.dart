...
import 'package:flexurio_chiron_accounting/src/app/view/page/material_stock_rupiah/util/pdf_material_stock_rupiah.dart';
...
class MaterialStockRupiahExportPdfForm extends StatefulWidget {
  const MaterialStockRupiahExportPdfForm._();

  static Widget prepare() {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) => MaterialStockRupiahQueryBloc(),
        ),
        BlocProvider(
      ...
      child: const MaterialStockRupiahExportPdfForm._(),
    );
  }

  @override
  State<MaterialStockRupiahExportPdfForm> createState() =>
      _MaterialStockRupiahExportPdfFormState();
}

class _MaterialStockRupiahExportPdfFormState
    extends State<MaterialStockRupiahExportPdfForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<MaterialStockRupiahQueryBloc,
        ...
        state.maybeWhen(
          ...
          loaded: (pageOptions) async {
            final pdf = pw.Document()
              ..addPage(
                await pdfMaterialStockInRupiah(
                  data: pageOptions.data,
                ),
              );
            await Printing.sharePdf(
              bytes: await pdf.save(),
              filename: 'Material_Stock_In_Rupiah.pdf',
            );
      ...
      child: CardForm(
        popup: true,
        title: '${action.title} ${'material_stock_in_rupiah'.tr()}',
        icon: action.icon,
        actions: [
          Button(
            permission: null,
          ...
          const Gap(12),
          BlocBuilder<MaterialStockRupiahQueryBloc,
              MaterialStockRupiahQueryState>(
